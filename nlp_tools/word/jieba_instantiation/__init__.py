...
'''
def posseg_cut(tok ,sentence, HMM=True, use_paddle=False):
    """
    ...
    for w in tok.cut(sentence, HMM=HMM):
        yield w
    '''
    else:
...
def lcut(sentence, HMM=True, use_paddle=False):
    if use_paddle:
        return list(posseg_cut(sentence, use_paddle=True))
    return list(posseg_cut(sentence, HMM))
